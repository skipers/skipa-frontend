...
const competitorData = computed(() => ({
  ...
  datasets: [
    ...
    { label: '경쟁사', data: [5, 7, 6, 3, 5], backgroundColor: '#3B82F6', borderRadius: 4 },
  ],
}))
...
const expiringList = computed(() =>
  PATENTS.filter((p) => p.status === '만료 예정').sort((a, b) => a.expiryDate.localeCompare(b.expiryDate))
)
</script>

<template>
  <AppLayout title="만료 예정 관리 (Legal)">
    <div class="grid grid-cols-2 gap-4 mb-4">
      ...
      <AppCard title="자사 vs 경쟁사 특허 현황">
        ...
        <div class="mt-3 px-3 py-2 rounded-lg text-xs" style="background:rgba(234,0,44,0.06); color:#EA002C;">
